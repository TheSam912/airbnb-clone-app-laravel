<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <div>
                <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">Stays</h2>
                <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">Browse published listings.</p>
            </div>
        </div>
    </x-slot>
    <div class="py-4 px-4">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <form method="GET" action="{{ route('listings.index') }}"
                class="mb-6 bg-white dark:bg-gray-800 border dark:border-gray-600 rounded-2xl p-4 shadow-sm">
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-12 gap-3 items-end">
                    ...
                    <div class="lg:col-span-2 flex gap-2">
                        <button
                            class="w-full px-4 py-3 rounded-xl bg-rose-600 text-white font-medium hover:bg-rose-700">
                            Search
                        </button>

                        <a href="{{ route('listings.index') }}"
                            class="w-full px-4 py-3 rounded-xl border dark:border-rose-500 text-center hover:bg-gray-50 text-rose-500">
                            Reset
                        </a>
                    </div>
                </div>
            ...
            <div class="flex items-center justify-between mt-6">
                ...
                <a href="{{ $showMap ? $hideMapUrl : $showMapUrl }}"
                    class="hidden lg:inline-flex px-4 py-2 rounded-xl border dark:border-rose-500 dark:text-rose-500 hover:bg-gray-50">
